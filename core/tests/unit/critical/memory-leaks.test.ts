...
describe('Memory Leak Prevention', () => {
  ...
  describe('Event Listener Management', () => {
    it('should not accumulate message event listeners', () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (
          typeof (tempClient as { destroy?: () => void }).destroy === 'function'
        ) {
          (tempClient as { destroy: () => void }).destroy();
        }
      }
  ...
  describe('Message Queue Memory Management', () => {
    it('should not accumulate messages indefinitely in queue', () => {
      ...
      expect((queue as { queue?: unknown[] }).queue?.length || 0).toBe(0);
    });
    ...
    it('should clear pending messages on queue reset', () => {
      ...
      expect((queue as { queue?: unknown[] }).queue?.length).toBeGreaterThan(0);
  ...
  describe('DOM Reference Management', () => {
    ...
    it('should clean up iframe references on client disposal', () => {
      ...
      const iframeRef = (client as { iframe?: HTMLIFrameElement }).iframe;
  ...
  describe('Callback and Promise Management', () => {
    it('should not accumulate unresolved promises', async () => {
      ...
      for (let i = 0; i < 100; i++) {
        promises.push(client.requestObjectTree().catch((): null => null));
      }
    ...
    it('should clean up one-time event listeners after use', async () => {
      ...
      const promise = responseDispatcher.once(
        VirtualDisplayResponseType.OBJECT_TREE
      );
  ...
  describe('Circular Reference Prevention', () => {
    it('should handle circular references without memory leaks', () => {
      ...
      const circularStates: Array<
        State & { self?: unknown; [key: string]: unknown }
      > = [];

      for (let i = 0; i < 10; i++) {
        const obj: State & { self?: unknown; [key: string]: unknown } = {
          attributes: [{ name: 'Test', values: [] }],
        };
        obj.self = obj; // Circular reference
        (obj.attributes[0] as { parent?: unknown }).parent = obj; // Another circular reference
  ...
  describe('Resource Monitoring', () => {
    it('should limit message queue size', () => {
      ...
      const queueLength = (queue as { queue?: unknown[] }).queue?.length || 0;
