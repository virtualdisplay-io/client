...
describe('Camera Config - Zoom', () => {
  beforeEach(setupTest);
  afterEach(teardownTest);

  it('should handle standard zoom range', async () => {
    client = createClientWithCamera({
      initialZoom: 100,
      minZoom: 25,
      maxZoom: 400,
    ...
    const configMessage = findConfigMessage();
    expect(configMessage?.config.camera?.minZoom).toBe(25);
    expect(configMessage?.config.camera?.maxZoom).toBe(400);
  });

  it('should handle custom zoom for large models', async () => {
    client = new VirtualdisplayClient({
      parent: '#test-viewer',
      model: 'large-solar-panel',
      license: 'test',
      camera: {
        initialZoom: 270,
        minZoom: 25,
    ...
    const configMessage = findConfigMessage();
    expect(configMessage?.config.camera?.initialZoom).toBe(270);
  });
});

// Tilt constraint tests
describe('Camera Config - Tilt', () => {
  beforeEach(setupTest);
  afterEach(teardownTest);

  it('should send tilt limits in CONFIG message', async () => {
    client = createClientWithCamera({
      minTilt: -45,
      maxTilt: 90,
    });
    ...
    const configMessage = findConfigMessage();
    expect(configMessage?.config.camera?.minTilt).toBe(-45);
    expect(configMessage?.config.camera?.maxTilt).toBe(90);
  });
});
